...
class ode:
    ...
    def __init__(
        ...
        *,
        ...
        # Allow t0 > t1 for reverse (data -> noise) integration
        assert t0 != t1, "ODE sampler needs distinct endpoints"
        if t0 > t1:
            t0, t1 = t1, t0  # linspace(t0, t1) must go low -> high
        self.drift = drift
        # num_steps=1 means 1 euler step, need 2 time points (start + end)
        num_t_points = max(num_steps, 2)
        self.t = 1 - th.linspace(t0, t1, num_t_points)
        if use_time_shift:
            self.t = time_dist_shift * self.t / (1 + (time_dist_shift - 1) * self.t)
        else:
            logger.warning("Sampling Time shift is disabled.")
