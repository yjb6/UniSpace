#!/usr/bin/env python3
"""
Zero-shot evaluation script for VTP.

Modified from OpenCLIP.

Usage:
    python tools/test_zero_shot_hf.py --model_path /path/to/vtp_hf_model --data_path /path/to/imagenet/val
...
def build_zero_shot_classifier(
    ...
    Args:
        model: VTPModel instance with get_clip_text_feature method.
    ...
    def _process_batch(batch_classnames: List[str]) -> torch.Tensor:
        ...
        texts = [template(c) for c in batch_classnames for template in templates]
        tokens = tokenizer(texts).to(device)

        # Use VTPModel's get_clip_text_feature method
        text_features = model.get_clip_text_feature(tokens, normalize=True)
...
def evaluate(
    ...
    with torch.inference_mode():
        for images, targets in tqdm(dataloader, desc="Evaluating"):
            ...
            with autocast_ctx():
                image_features = model.get_clip_image_feature(images, normalize=True)
...
def main():
    parser = argparse.ArgumentParser(description="Zero-shot ImageNet evaluation for VTP HuggingFace model")
    ...
    print("=" * 60)
    print("Zero-Shot ImageNet Evaluation (VTP HuggingFace)")
    print("=" * 60)
    ...
    context_length = model.config.text_context_length
    print(f"Image size: {image_size}")
    print(f"Context length: {context_length}")

    # Load tokenizer
    print("Loading tokenizer...")
    tokenizer = get_tokenizer('ViT-B-32', context_length=context_length)
