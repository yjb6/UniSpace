...
def validate_geneval_snapshot(imagedir):
    """Require the canonical 553 prompts with exactly four PNGs each."""
    prompt_ids = sorted(
        int(name) for name in os.listdir(imagedir)
        if name.isdigit() and os.path.isdir(os.path.join(imagedir, name))
    )
    expected_ids = list(range(553))
    if prompt_ids != expected_ids:
        missing = sorted(set(expected_ids) - set(prompt_ids))
        extra = sorted(set(prompt_ids) - set(expected_ids))
        raise RuntimeError(
            f"GenEval prompt coverage mismatch: prompts={len(prompt_ids)}/553 "
            f"missing={missing[:20]} extra={extra[:20]}"
        )
    expected_samples = [f"{index}.png" for index in range(4)]
    invalid = []
    for prompt_id in prompt_ids:
        sample_dir = os.path.join(imagedir, str(prompt_id), "samples")
        samples = sorted(
            name for name in os.listdir(sample_dir)
            if os.path.isfile(os.path.join(sample_dir, name)) and name.endswith(".png")
        ) if os.path.isdir(sample_dir) else []
        if samples != expected_samples:
            invalid.append((prompt_id, samples))
    if invalid:
        raise RuntimeError(f"Invalid GenEval sample directories: {invalid[:10]}")
...
def load_models(args):
    ...
    CKPT_PATH = os.path.join(args.model_path, "mask2former_swin-s-p4-w7-224_8xb2-lsj-50e_coco_20220504_001756-c9d0c4f2.pth")
    object_detector = init_detector(CONFIG_PATH, CKPT_PATH, device=DEVICE)

    clip_arch = args.options.get('clip_model', "ViT-L-14")
    clip_pretrained = os.environ.get("GENEVAL_CLIP_PRETRAINED", "openai")
...
def evaluate_image(filepath, metadata):
    ...
    try:
        ...
        with open(filepath, 'rb') as f:
            header = f.read(8)
            if len(header) < 8:
                print(f"Warning: Invalid image file (too small): {filepath}")
                return None
    ...
    if hasattr(result, 'pred_instances'):
        ...
        bboxes = scores = labels = masks = None
    ...
    for class_idx, classname in enumerate(classnames):
        ...
        while ordering:
            ...
            # 组合 bbox 和 score (保持原有格式兼容性)
            bbox_with_score = np.append(class_bboxes[max_obj], class_scores[max_obj])
            mask = None if class_masks is None else class_masks[max_obj]
            detected[classname].append((bbox_with_score, mask))

            # NMS 过滤
            ordering = [
                obj for obj in ordering
                if NMS_THRESHOLD == 1 or compute_iou(class_bboxes[max_obj], class_bboxes[obj]) < NMS_THRESHOLD
            ]
...
def evaluate_image_old(filepath, metadata):
    result = inference_detector(object_detector, filepath)
...
if __name__ == "__main__":
    ...
    subfolders = validate_geneval_snapshot(args.imagedir)
    ...
    for subfolder in tqdm(subfolders[start:end]):
        ...
        with open(os.path.join(folderpath, "metadata.jsonl"), encoding='utf-8') as fp:
    ...
    if rank == 0:
        ...
        filenames = [result.get('filename') for result in final_results if isinstance(result, dict)]
        if (len(final_results) != 2212 or len(filenames) != 2212 or
                len(set(filenames)) != 2212 or any(result is None for result in final_results)):
            raise RuntimeError(
                f"GenEval result coverage mismatch: rows={len(final_results)} "
                f"valid={len(filenames)} unique={len(set(filenames))}"
            )
        # Save results
