...
from modeling.unimm import (
    UnimmConfig, Unimm, SiglipVisionConfig, SiglipVisionModel,
)
from modeling.unimm.qwen3_vl import Qwen3VLConfig, Qwen3VLForConditionalGeneration, NaiveCache
...
def setup_models(model_path, llm_path, vae_path, device=0, vae_type='flux1-vae'):
    """
    ...
    # Load LLM
    llm_config = Qwen3VLConfig.from_pretrained(llm_path)
    llm_config.text_config.qk_norm = True
    llm_config.tie_word_embeddings = False
    llm_config._attn_implementation = 'sdpa'

    # Load VAE
    vae_model, vae_config = load_ae(local_path=vae_path, vae_type=vae_type)
    ...
    # Create fusion model
    language_model = Qwen3VLForConditionalGeneration.from_pretrained(llm_path, config=llm_config)
    processor = AutoProcessor.from_pretrained(llm_path)
    tokenizer = processor.tokenizer

    model = Unimm(language_model, None, tokenizer, config)
...
def editing_with_text_img_cfg(
    ...
    past_key_values = NaiveCache(model.config.llm_config.text_config.num_hidden_layers)
    newlens = [0]
    new_rope = [0]
...
The planning process is enclosed within <think> </think> tags, i.e. <think> planning process here </think> image here'''
    ...
    # Setup for image CFG
    cfg_img_past_key_values = NaiveCache(model.config.llm_config.text_config.num_hidden_layers)
    cfg_img_newlens = [0]
    cfg_img_new_rope = [0]
...
def main():
    ...
    parser.add_argument("--model_path", type=str, required=True,
                        help="Path to unimm model")
    parser.add_argument("--llm_path", type=str, required=True, help="Path to LLM pretrained weights")
    parser.add_argument("--vae_path", type=str, required=True, help="Path to VAE weights")
    parser.add_argument("--vae_type", type=str, default="flux1-vae", choices=["flux1-vae", "flux2-vae"],
    ...
    model, vae_model, tokenizer, new_token_ids, vae_transform, vit_transform = setup_models(
        args.model_path, args.llm_path, args.vae_path, args.device,
        vae_type=getattr(args, 'vae_type', 'flux1-vae'),
    )
